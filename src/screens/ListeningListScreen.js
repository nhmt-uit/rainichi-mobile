...
import {loadMoreListeningList} from '../redux/actions';
...
// Ported from the old app's ListeningListScreen.js + base/LoadedListScreen.js
// (same base class ConversationListScreen.js already ported standalone from).
export default function ListeningListScreen({navigation, route}) {
  const dispatch = useDispatch();
  const listeningList = useSelector(state => state.Listening.listeningList);
  const nextUrl = useSelector(state => state.Listening.nextUrl);
  const isLoading = useSelector(state => state.Listening.isListeningLoading);

  const [data, setData] = useState(listeningList);
  ...
  React.useEffect(() => {
    if (listeningList != null && prevListeningList.current !== listeningList) {
      prevListeningList.current = listeningList;
      setData(prev => [...prev, ...listeningList]);
    }
  }, [listeningList]);

  function onEndReached() {
    if (nextUrl && !isLoading && !onEndReachedCalledDuringMomentum.current) {
      dispatch(loadMoreListeningList(nextUrl));
